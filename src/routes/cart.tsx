        ...
        {count === 0 ? (
          <div className="mt-10 overflow-hidden rounded-[2rem] border border-white/10 bg-gradient-to-br from-secondary/60 via-card to-background p-12 text-center">
            ...
            <div className="mt-8 flex flex-wrap justify-center gap-3">
              <Button asChild size="lg" className="rounded-2xl mint-glow">
                <Link to="/shop">Browse catalogue <ArrowRight className="ml-2 h-4 w-4" /></Link>
              </Button>
          ...
          <div className="mt-8 grid gap-8 md:grid-cols-[1fr,340px]">
            <ul className="space-y-3">
              {items.map((i) => (
                <li key={i.id} className="flex gap-4 rounded-2xl border border-white/10 bg-card/60 p-3 backdrop-blur transition-colors hover:border-primary/30">
                  ...
                  <div className="flex min-w-0 flex-1 flex-col">
                    <div className="flex items-start justify-between gap-2">
                      <div className="min-w-0">
                        ...
                        <p className="text-xs text-muted-foreground">{formatGHS(i.price_ghs)} · per {i.unit}</p>
                      </div>
                      <button onClick={() => remove(i.id)} aria-label="Remove" className="rounded-lg p-1.5 text-muted-foreground transition-colors hover:bg-destructive/10 hover:text-destructive">
                        <Trash2 className="h-4 w-4" />
                      </button>
                    </div>
                    <div className="mt-auto flex items-center justify-between pt-3">
                      <div className="flex items-center rounded-xl border border-white/10 bg-background/50">
                        <button className="p-2 transition-colors hover:text-primary" onClick={() => setQty(i.id, i.quantity - 1)}><Minus className="h-3.5 w-3.5" /></button>
                        <span className="w-9 text-center text-sm font-semibold tabular-nums">{i.quantity}</span>
                        <button className="p-2 transition-colors hover:text-primary" onClick={() => setQty(i.id, i.quantity + 1)}><Plus className="h-3.5 w-3.5" /></button>
                      </div>
                      <span className="font-display text-lg font-bold text-primary tabular-nums">{formatGHS(i.price_ghs * i.quantity)}</span>
                    </div>
                  </div>
                </li>
              ))}
            ...
            <aside className="h-fit rounded-2xl border border-border bg-card p-6 shadow-xs">
              ...
              <dl className="mt-4 space-y-3 text-sm font-sans">
                <div className="flex justify-between"><dt className="text-muted-foreground">Subtotal</dt><dd className="font-semibold text-foreground tabular-nums">{formatGHS(subtotal)}</dd></div>
                <div className="flex justify-between text-xs text-muted-foreground"><dt>Delivery</dt><dd>Calculated at checkout</dd></div>
                <div className="mt-3 border-t border-border pt-3 flex items-baseline justify-between">
                  ...
                  <dd className="font-display text-2xl font-bold text-primary tabular-nums">{formatGHS(subtotal)}</dd>
                </div>
              </dl>
              ...
              {(() => {
                ...
                const formattedItems = items
                  .map((item) => `- ${item.quantity} x ${item.name} (${item.unit}) - ${formatGHS(item.price_ghs * item.quantity)}`)
                  .join("\n");
                ...
                return (
                  <Button asChild size="lg" variant="outline" className="mt-3 w-full rounded-xl border-emerald-600/30 text-emerald-600 hover:bg-emerald-50 dark:hover:bg-emerald-950/20 dark:text-emerald-400 font-semibold transition-all">
              ...
              <p className="mt-4 text-center text-[10px] tracking-wide text-muted-foreground font-sans uppercase">MoMo · Paystack · COD</p>
            </aside>
          </div>
        )}
    </div>
