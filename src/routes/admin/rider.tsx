...
export const Route = createFileRoute("/admin/rider")({
  head: () => ({ meta: [{ title: "Rider Delivery Console" }, { name: "robots", content: "noindex" }] }),
...
function RiderPortalPage() {
  ...
  const handleUpdate = async (id: string, newStatus: "out_for_delivery" | "delivered") => {
    try {
      ...
      toast.success(newStatus === "out_for_delivery" ? "Delivery started!" : "Delivery completed successfully!");
    ...
    <AdminShell>
      <div className="space-y-6">
        ...
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">Active delivery runs, customer location pins, and fulfillment statuses.</p>
        </div>
        ...
        <div className="rounded-2xl border border-border bg-card p-5 shadow-xs flex items-center justify-between">
          <div>
            <p className="text-xs font-bold uppercase tracking-wider text-muted-foreground">Active Deliveries</p>
            <p className="font-display text-3xl font-bold text-foreground mt-1">{activeDeliveries.length} runs</p>
          </div>
          <span className={`h-2.5 w-2.5 rounded-full bg-emerald-500 ${activeDeliveries.length > 0 ? "animate-ping" : ""}`} />
        </div>
        ...
        <div className="space-y-4">
          {isLoading ? (
            ...
            return (
              <div key={o.id} className="rounded-2xl border border-border bg-card shadow-xs overflow-hidden transition-all duration-300">
                ...
                <div className="flex items-center justify-between border-b bg-muted/40 px-5 py-4">
                  <div>
                    ...
                    <span className="text-xs text-muted-foreground ml-2">({o.delivery_type})</span>
                  </div>
                ...
                <div className="p-5 space-y-4">
                  ...
                  <div className="flex items-center justify-between gap-4">
                    <div>
                      <h3 className="font-semibold text-foreground text-base">{o.customer_name}</h3>
                    ...
                    <div className="grid gap-2 sm:grid-cols-2 pt-1.5 border-t border-border/50">
                      ...
                      {o.ghana_post_gps && (
                        <div className="flex items-center justify-between gap-3 text-xs bg-card border rounded-lg p-2">
                          <span className="font-semibold text-muted-foreground truncate">GPS: {o.ghana_post_gps}</span>
                          <Button variant="ghost" size="icon" className="h-6 w-6" onClick={() => copyToClipboard(o.ghana_post_gps, "GPS Address")}>
                      ...
                      {o.gps_coordinates ? (
                        <Button asChild variant="outline" size="sm" className="rounded-lg text-xs font-semibold flex items-center justify-center gap-1">
                          <a href={`https://www.google.com/maps/search/?api=1&query=${o.gps_coordinates}`} target="_blank" rel="noopener noreferrer">
                            <Navigation className="h-3.5 w-3.5 text-primary" /> Navigate on Google Maps
                          </a>
                        </Button>
                      ) : (
                  ...
                  <div className="border-t pt-3">
                    ...
                    {isExpanded && (
                      <div className="mt-3 bg-secondary/20 border border-border/40 rounded-xl p-3 space-y-2 text-xs">
                        ...
                        {o.order_items && o.order_items.map((item: any, idx: number) => (
                          <div key={idx} className="flex justify-between">
                            <span>{item.quantity} × {item.product_name} ({item.unit})</span>
                            <span className="font-semibold">{formatGHS(Number(item.line_total_ghs))}</span>
                          </div>
                        ))}
                        <div className="flex justify-between border-t pt-2 font-bold text-foreground">
                          <span>Total to collect ({o.payment_method.replace(/_/g, " ").toUpperCase()})</span>
          ...
          {!isLoading && activeDeliveries.length === 0 && (
            <div className="rounded-2xl border border-dashed border-border/80 bg-card py-16 text-center shadow-xs">
              ...
              <h4 className="font-display text-base font-bold text-foreground">No active deliveries</h4>
