...
CREATE POLICY "read own order items" ON public.order_items FOR SELECT USING (
  EXISTS (SELECT 1 FROM public.orders o WHERE o.id = order_id AND (o.user_id IS NULL OR o.user_id = auth.uid() OR public.has_role(auth.uid(),'admin')))
);
CREATE POLICY "guests insert order items" ON public.order_items FOR INSERT TO anon WITH CHECK (
  EXISTS (SELECT 1 FROM public.orders o WHERE o.id = order_id AND o.user_id IS NULL)
);
CREATE POLICY "auth insert order items" ON public.order_items FOR INSERT TO authenticated WITH CHECK (
